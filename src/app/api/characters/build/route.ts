...
export async function POST(req: NextRequest) {
  try {
    ...
    // Merge animal character data into customizations so we can read it in prompt engine
    const mergedCustomizations = {
      ...(customizations || {}),
      ...(characterType === 'animal' ? { characterType, selectedAnimal, animalMode } : {}),
    }

    const data = {
      name: name.trim(),
      selections: JSON.parse(JSON.stringify(selections || {})),
      customizations: JSON.parse(JSON.stringify(mergedCustomizations)),
