...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectId, postId, vibe } = await req.json()
    if (!projectId || !postId) {
    ...
    // Build the scene prompt. A user-supplied vibe overrides the default rotating scene.
    const sceneIdx = DAY_INDEX[post.dayLabel] ?? 0
    const scene = (vibe && String(vibe).trim()) ? String(vibe).trim() : SCENES[sceneIdx]
