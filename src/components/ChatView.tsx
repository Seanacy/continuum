                ...
                {msg.role === 'assistant' && msg.contentPack && (
                  <div className="mb-3 space-y-2 overflow-hidden">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-base font-semibold text-continuum-accent uppercase tracking-wider">Content Pack</span>
                      <span className="text-sm text-continuum-muted">{msg.contentPack.pieces.length} pieces · {msg.contentPack.weekTheme}</span>
                    </div>
                    <p className="text-sm text-continuum-muted italic mb-1">Swipe to see more cards &#8594;</p>
                        ...
                        <div key={idx} className="snap-start shrink-0 w-64 p-3 rounded-xl bg-continuum-surface border border-continuum-border">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-1.5">
                              <span className="text-sm font-semibold text-continuum-accent uppercase">{piece.contentType.replace(/_/g, ' ')}</span>
                              {piece.platform && <span className="text-sm text-continuum-muted">· {piece.platform}</span>}
                            </div>
                            {piece.daySuggestion && <span className="text-sm text-continuum-muted">{piece.daySuggestion}</span>}
                          </div>
                          <p className="text-base text-continuum-text whitespace-pre-wrap leading-relaxed mb-2 line-clamp-6">{piece.content}</p>
                          {piece.hashtags && piece.hashtags.length > 0 && (
                            <p className="text-sm text-continuum-accent mb-2">{piece.hashtags.map((h: string) => h.startsWith('#') ? h : '#' + h).join(' ')}</p>
                          )}
                          {piece.needsUserPhoto && piece.photoSuggestion && (
                            <div className="flex items-center gap-1.5 text-sm text-amber-400 mb-2 p-1.5 rounded-lg bg-amber-500/10">
                              <span>📷</span>
                              <span>{piece.photoSuggestion}</span>
                            </div>
                          )}
                          {!piece.needsUserPhoto && piece.photoSuggestion && (
                            <div className="flex items-center gap-1.5 text-sm text-continuum-muted mb-2 p-1.5 rounded-lg bg-continuum-accent/5">
                              <span>🎨</span>
